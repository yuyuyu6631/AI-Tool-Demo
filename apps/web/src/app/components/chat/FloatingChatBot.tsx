...
const FEATURES = [
    { icon: Search, title: "找工具组合", body: "匹配最佳工具组合", color: "from-cyan-400 to-blue-500" },
    { icon: Scale, title: "做同类对比", body: "多维度对比优劣", color: "from-violet-400 to-fuchsia-500" },
];
        ...
        <>
            {isOpen && (
                <section
                    ref={panelRef}
                    style={panelStyle}
                    className="fixed z-50 flex h-[min(620px,calc(100vh-40px))] w-[min(430px,calc(100vw-32px))] flex-col overflow-hidden rounded-[24px] border border-cyan-300/35 bg-[#050816]/96 text-white shadow-[0_0_0_1px_rgba(124,58,237,0.38),0_0_34px_rgba(37,99,235,0.26),0_0_58px_rgba(168,85,247,0.18)] backdrop-blur-xl sm:h-[min(660px,calc(100vh-48px))] sm:w-[430px]"
                >
                    <div className="absolute inset-0 bg-[linear-gradient(120deg,rgba(14,165,233,0.10),transparent_42%,rgba(147,51,234,0.14)),radial-gradient(circle_at_50%_0%,rgba(59,130,246,0.18),transparent_38%)]" />
                    ...
                    <header
                        ...
                        className="relative cursor-grab select-none border-b border-white/10 bg-[#0c1736]/82 px-4 py-4 active:cursor-grabbing sm:px-5"
                        title="拖动调整位置"
                    >
                        <div className="flex items-start justify-between gap-3">
                            <div className="flex min-w-0 items-center gap-3">
                                <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl border border-cyan-200/40 bg-gradient-to-br from-cyan-400 via-blue-600 to-fuchsia-600 shadow-[0_0_22px_rgba(34,211,238,0.36)]">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="mt-1 truncate text-sm text-slate-300">
                                        <span className="font-semibold text-cyan-300">3 秒</span> 帮你找到能用的 AI 工具
                                    </p>
                                </div>
                            </div>
                    ...
                    >
                        {!hasConversation ? (
                            <div className="space-y-3">
                                <div className="rounded-[20px] border border-violet-400/45 bg-[#0a1028]/78 p-3.5 shadow-[inset_0_0_28px_rgba(59,130,246,0.12)]">
                                    ...
                                    <div className="mt-4 grid grid-cols-2 gap-3">
                                        {FEATURES.map((item) => {
                                            ...
                                            return (
                                                <div className="min-w-0 rounded-2xl border border-white/10 bg-white/5 p-3" key={item.title}>
                                                    <div className={`mb-3 grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br ${item.color} p-px`}>
                                                        <div className="grid h-full w-full place-items-center rounded-[11px] bg-[#071733]/88">
                            ...
                            <div className="space-y-4 pb-2">
                                {messages.map((msg: Message, idx: number) => (
                                    <div key={`${msg.role}-${idx}`} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                                        <div className={`grid h-8 w-8 shrink-0 place-items-center rounded-2xl text-xs font-semibold ${msg.role === "user" ? "bg-white/12 text-white" : "bg-gradient-to-br from-cyan-400 to-violet-600 text-white"}`}>
                                            {msg.role === "user" ? "我" : "AI"}
                                        </div>
                                        <div className={`max-w-[82%] rounded-3xl px-4 py-3 text-sm leading-6 shadow-lg ${msg.role === "user" ? "rounded-tr-md bg-cyan-400 text-slate-950" : "rounded-tl-md border border-white/10 bg-white/8 text-slate-100"}`}>
                                ...
                                {isLoading && messages[messages.length - 1]?.role !== "assistant" && (
                                    <div className="flex gap-3">
                                        <div className="grid h-8 w-8 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-cyan-400 to-violet-600 text-xs font-semibold text-white">AI</div>
                                        <div className="flex items-center gap-2 rounded-3xl rounded-tl-md border border-white/10 bg-white/8 px-4 py-3">
                    ...
                    <footer className="relative border-t border-white/10 px-4 pb-4 pt-3">
                                ...
                                {QUICK_QUESTIONS.map((question, index) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className="group flex min-h-11 items-center justify-between rounded-2xl border border-violet-400/40 bg-[#0b122d]/76 px-3 text-left text-sm font-medium text-slate-100 transition hover:border-cyan-300/70 hover:bg-blue-500/12"
                                        >
                        ...
                        <form onSubmit={onSubmit} className="flex items-center gap-2 rounded-2xl border border-violet-400/50 bg-white/6 p-1.5 shadow-[inset_0_0_20px_rgba(59,130,246,0.10)] focus-within:border-cyan-300/80">
                            <input
                                type="text"
                                value={input}
                            ...
                            <button
                                ...
                                className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-gradient-to-br from-cyan-400 via-blue-600 to-fuchsia-600 text-white shadow-[0_0_22px_rgba(34,211,238,0.40)] transition hover:scale-105 disabled:scale-100 disabled:cursor-not-allowed disabled:opacity-45"
                                aria-label="发送消息"
                            >
            ...
            {!isOpen && (
                <button
                    ...
                    className="fixed bottom-6 right-6 z-50 flex items-center gap-2 rounded-full border border-cyan-300/25 bg-[#050816]/92 px-5 py-3 text-white shadow-[0_0_28px_rgba(59,130,246,0.32)] backdrop-blur transition hover:-translate-y-1 hover:border-cyan-300/55"
                    aria-label="打开 AI 助手"
                >
